...
; Write time series:
 ...
 if iter eq 0 then begin
  f1micro = fc(i).f*(fac/facuse) - f_resolution * 1.2 ; freq range in microHz
  f2micro = fc(i).f*(fac/facuse) + f_resolution * 1.2 ; freq range in microHz
  error_mhz = f_resolution * 0.001 ; accepted error in microHz
 endif

 if iter ge 1 then begin
  dfreq = abs(fc(i).f*(fac/facuse) - ff(0) )
  if dfreq lt 0.00005 then dfreq = 0.00005
  f1micro = ff(0) - dfreq  ; f_resolution * 0.01 ; freq range in microHz
  f2micro = ff(0) + dfreq  ; f_resolution * 0.01 ; freq range in microHz
  error_mhz = 0.0000005 ; accepted error in microHz
 endif
 ...
 print,' %%% f = ' + string(fc(i).f *(fac/facuse),format='(F10.5)') + ' micHz ' + $
   ...
   ' micHz : resol. in spec: ' +  string(error_mhz,format='(D11.8)')
 print,' n = ',npuse
