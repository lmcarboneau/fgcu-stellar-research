...
;   minfreq = 13.     & maxfreq  = 23.      ; freq. range in c/day
;   minfreq = 3.     & maxfreq  = 50.      ; freq. range in c/day
   minfreq = 0.0     & maxfreq  = 550./11.5741 ; freq. range in c/day
;   minfreq = 90./11.5741  & maxfreq  = 250/11.5741 ; freq. range in c/day
; -------------------------------------------------------------------
...
dmax = 150000L ; max number of data points in light curves
; -------------------------------------------------------------------

; -------------------------------------------------------------------
remove_spline_fit_to_scattered_light = 1 ; spline fit to data w/ scat light problem?
remove_scattered_light_data = 0 ; completely remove data w/ scat light problem?
data_rel_to_main_target = 0 ; always == 0 ? ---> not fully implem. !!
avoid_xyoff = 0 ; avoid data for the first 5 days/nights == sign. telescope offsets ?
remove_offsets = 30.0 ; if > 0 then remove offsets, eg. 30 ==> rem. offset for every 30 orbits!
subtract_osc = 1 ; subtract know osc. for Altair?
; -------------------------------------------------------------------
...
suff = string(remove_spline_fit_to_scattered_light,format='(I1)') + $
 ...
 strcompress(string(remove_offsets,format='(I3)'),/remove_all) + '_OscOff' + $
 string(subtract_osc,format='(I1)') + '_f' + $
...
; restore,'/ai39/bruntt/wire/altair/altair_merged_allslots_xx_decor1.idl'
restore,'/ai39/bruntt/wire/altair/altair_merged_allslots_31_decor1.idl' ; 3.10.03
nstar = n_elements(wire3(0).mag)
t0 = 51480.0D ; round(median(wire3.hjd))
...
wday = where(wire3.hjd gt -100.,c)
minday = round(min(wire3(wday).hjd-t0))
maxday = round(max(wire3(wday).hjd-t0))

minday = -4 ; range of time in the plot !
maxday =  4 ; range of time in the plot !

print,' %%% From day '+strcompress(minday)+' to day ' + strcompress(maxday)

; -------------------------------------------------------------------

if n_elements(wireamp) le 1 then goto,dopl
; -------------------------------------------------------------------
...
; wg = where(wire3.mag(i) gt 0. and wire3.mag(i) lt 20.,c)
; mmag = median(wire3(wg).mag(i))
; nn = robust_sigma(wire3(wg).mag(i))

; wg2 = where((wire3.mag(i)-mmag) lt 20.*nn,cg2)
; wg = wg(wg2)

; Debug:
; plot,wire3(wg).hjd,wire3(wg).mag(0),psym=3,yr=[-1,1]*20.*nn+mmag,ysty=3
...
; plot,wire3(wg).hjd-t0,wire3(wg).mag(i),yr=[-1,1]* nn * 5. + mmag,$
...
if n_elements(dummy) eq 0 then $
   readcol,'/ai39/bruntt/wire/altair/altair5.per',dummy,p98f,p98a,p98p,format='A,D,D,D'
...
if only_plot eq 1 then goto,dopl2
 ...
 wx = where(wire3.mag(i) gt 0. and wire3.mag(i) lt 20. and $
           (wire3.hjd-t0) gt (minhjd) and (wire3.hjd-t0) lt maxhjd,c) 
; exclude data where,xy-offsets were made?
 noise = robust_sigma(wire3(wx).mag(i))
 mmag = median(wire3(wx).mag(i))
 wx2 = where(abs(wire3(wx).mag(i)-mmag) lt 10.*noise,cg2)
 wx = wx(wx2) ; Remove 10 sigma outliers!
   ...
   if (data_rel_to_main_target eq 1) and (i ne 0) then $
      dat = dat - wire3(wx).mag(0)
   resistant_mean,dat,3,me,sd,nr
   dat = dat - median(dat) ; me

if subtract_osc eq 1 and i eq 0 then begin
   readcol,'/ai39/bruntt/wire/altair/altair5.per',dummy,p98f,p98a,p98p,format='A,D,D,D'
   nfreq = n_elements(p98f) & np = n_elements(tt)
   dart2 = fltarr(np)
   for pp=0, nfreq-1 do $
    dart2 = dart + p98a(pp) * sin(2. * !DPI * p98f(pp) * tt + p98p(pp) * (2. * !DPI)  ) 
   dat = dat - dart2
endif
...
if remove_offsets gt 0.0 then begin
 wire_remove_offsets,tt,dat,datx,remove_offsets ; remove offsets?
  ; if i eq 0 then stop ; HK stop ; plot,tt,dat-datx,psym=3,yr=[-1,1]*0.005
 dat = datx - median(datx)
 add_com = '' & if remove_offsets gt 1.0 then add_com = 's' 
 print,' %%% Offsets removed for every '+$
           strcompress(string(remove_offsets,format='(F9.1)'),/remove_all) + $
       ' orbit'+add_com + '!'
endif
