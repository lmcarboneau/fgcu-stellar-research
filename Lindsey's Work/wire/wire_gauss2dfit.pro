; $Id: gauss2dfit.pro,v 1.11 2003/02/03 18:13:14 scottm Exp $
;
; Copyright (c) 1995-2003, Research Systems, Inc.  All rights reserved.
...
Function wire_gauss2dfit, z, a, x, y, NEGATIVE = neg, TILT=tilt
;+
; NAME:
;	GAUSS2DFIT
;
...
xfit = wire_gaussfit(x, z[*,iy], ax, NTERMS=4) ;Guess at params by taking slices
yfit = wire_gaussfit(y, z[ix,*], ay, NTERMS=4)
...
result = wire_curvefit([nx, ny, x, y], reform(z, n, /OVERWRITE), $
   		     replicate(1.,n), a, itmax=50, $
		     function_name = "GAUSS2_FUNCT")

    if n_elements(result) eq 1 then begin
      app = string(result) & wa = where(strmatch(app,'*fail*',/fold) eq 1,ca)
      if ca ge 1 then begin ; fit failed
        yfit = 'Failed wire_curvefit in wire_gauss2dfit.pro !'
        goto,failed_gauss2dfit
      endif
   endif
...
; Successful fit:
yfit = REFORM(result, nx, ny, /OVERWRITE)
