function [XFtoG, ff] = find_periods_koi(FileNamesFile, KeplerDataFile)
 
%**********************Read Me!*************************************
%Text in this format is for questionable code or code tha is 
%not completely explained yet. which means....I have 
%little to no clue yet what it does :D
%*******************************************************************
...
for i = 1:numel(fileNames)
    ...
    if exist_check ~= 0
        ...
        fclose(fileNames_open);
            ...
            phase = (fitsFile(:,1)-KOI_Data(jj, 4)/KOI_Data(jj, 2));
            tr_phase_length = KOI_Data(jj, 3)/(24*KOI_Data(jj, 2));
            ...
            fileOutHeader = fopen('Matlab_Processed/Star_Data','a');
            fprintf(fileOutHeader, '%s  %s  %f  %f  %f  %f  %f\n\n', fileNames{i}, Range, zc, flicker, dataFile, det_stat, sigma);
            fileOutHeader = fclose(fileOutHeader);
